...
class LingTree:
   ...
   def write_paths(self,rl=True):
       l = list(self.collect_leaves(ignore_empties=True))
       s = []
       d = {}
       connectors = []
       for leaf in l:
           ...
           #s.append(" ".join(cur_s))
           d[leaf] = cur_s
       for i,leaf1 in enumerate(l):
            if i<len(l)-1:
                leaf2 = l[i+1]
                for id2,n in enumerate(d[leaf2]):
                    if n in d[leaf1]:
                        id1 = d[leaf1].index(n)
                        connectors.append((id1,id2))
                        break
       for i,(id1,id2) in enumerate(connectors):
           #print [x.name for x in d[l[i]]], [x.name for x in d[l[i+1]]]
           c = d[l[i]][id1]
           n = copy.deepcopy(c)
           if rl:
            n.name = n.name+"_r"
            d[l[i]] = d[l[i]][:id1] + [n] +  d[l[i]][id1+1:]
           else:
            n.name = "r"
            d[l[i]] = d[l[i]][:id1+1] + [n] + d[l[i]][id1 + 1:]
           c = d[l[i+1]][id2]
           n = copy.deepcopy(c)
           if rl:
            n.name = n.name+"_l"
            d[l[i+1]] = d[l[i+1]][:id2] + [n] +  d[l[i+1]][id2+1:]
           else:
            n.name =  "l"
            d[l[i + 1]] = d[l[i + 1]][:id2+1] + [n] + d[l[i + 1]][id2 + 1:]

           #print [x.name for x in d[l[i]]], [x.name for x in d[l[i+1]]]
       for leaf in l:
           s.append(" ".join([n.name for n in d[leaf]]))
       return "#".join(s)


   def write_tree(self,noft=False):
       s = "("
       if self.is_leaf():
           s+= self.value[0]
       else:
       ...
       for ch in self.childs:
           ...
           s+=ch.write_tree(noft)
       s += ")"
       return s

   # returns string represents the tree
   def __str__(self):
      return self.write_tree(noft=False)
